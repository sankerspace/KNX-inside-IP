...
function EIBConnection.__EIBGetBusmonitorPacketTS_Complete(self)
    if DEBUG then print("Entering EIBConnection.__EIBGetBusmonitorPacketTS_Complete()") end
    self.__complete = nil;
    if EIBConnection.__EIB_GetRequest(self) == -1 then 
      return -1;
    end
    if ( bit.bor(  bit.lshift(self.data[0+1], 8) , (self.data[0+2])) ~= 21) or (#(self.data) < 7) then 
      self.errno = "ECONNRESET"
      return -1
    end
    ...
    if self.ptr7 ~= nil then
      self.ptr7.data =  bit.bor( bit.lshift(self.data[3+1], 24), bit.bor( bit.lshift(self.data[3+2], 16), bit_bor( bit.lshift(self.data[3+3], 8), bit_bor(self.data[3+4])
    end
    self.buf.buffer = self.data:sub(7)
    return #(self.buf.buffer)

end

function EIBConnection.EIBGetBusmonitorPacketTS_async(self, status, timestamp, buf)
  if DEBUG then print("Entering EIBConnection.EIBGetBusmonitorPacketTS_async()") end
    ibuf = {}
    for i=1,2 do ibuf[i]="" end
    self.ptr2 = status
    self.ptr7 = timestamp
    self.buf = buf
    self.__complete = self:__EIBGetBusmonitorPacketTS_Complete();
    return 0
end

function EIBConnection.EIBGetBusmonitorPacketTS(self, status, timestamp, buf)
  local ibuf
    if DEBUG then print("Entering EIBConnection.EIBGetBusmonitorPacketTS()") end
    if self:EIBGetBusmonitorPacketTS_async (status, timestamp, buf) == -1  then 
      return -1
    end
    return self:EIBComplete()
  end
...
function EIBConnection.__EIBOpenBusmonitorTS_Complete(self)
    if DEBUG then print("Entering EIBConnection.__EIBOpenBusmonitorTS_Complete()") end
    self.__complete = nil;
    if EIBConnection.__EIB_GetRequest(self) == -1 then 
      return -1;
    end
    ...
    if ( bit.bor(  bit.lshift(self.data[0+1], 8) , (self.data[0+2])) ~= 22) or (#(self.data) < 6) then 
      self.errno = "ECONNRESET"
      return -1
    end
    if self.ptr7 ~= nil then
      self.ptr7.data =  bit.bor( bit.lshift(self.data[2+1], 24), bit.bor( bit.lshift(self.data[2+2], 16), bit_bor( bit.lshift(self.data[2+3], 8), bit_bor(self.data[2+4])
    end
    return 0

end

function EIBConnection.EIBOpenBusmonitorTS_async(self, timebase)
  if DEBUG then print("Entering EIBConnection.EIBOpenBusmonitorTS_async()") end
    ibuf = {}
    for i=1,2 do ibuf[i]="" end
    self.ptr7 = timebase
    ibuf[1] = 0
    ...
    self.__complete = self:__EIBOpenBusmonitorTS_Complete();
    return 0
end

function EIBConnection.EIBOpenBusmonitorTS(self, timebase)
  local ibuf
    if DEBUG then print("Entering EIBConnection.EIBOpenBusmonitorTS()") end
    if self:EIBOpenBusmonitorTS_async (timebase) == -1  then 
      return -1
    end
    return self:EIBComplete()
  end
...
function EIBConnection.__EIBOpenVBusmonitorTS_Complete(self)
    if DEBUG then print("Entering EIBConnection.__EIBOpenVBusmonitorTS_Complete()") end
    self.__complete = nil;
    if EIBConnection.__EIB_GetRequest(self) == -1 then 
      return -1;
    end
    ...
    if ( bit.bor(  bit.lshift(self.data[0+1], 8) , (self.data[0+2])) ~= 23) or (#(self.data) < 6) then 
      self.errno = "ECONNRESET"
      return -1
    end
    if self.ptr7 ~= nil then
...
function EIBConnection.EIBOpenVBusmonitorTS_async(self, timebase)
  if DEBUG then print("Entering EIBConnection.EIBOpenVBusmonitorTS_async()") end
    ibuf = {}
    for i=1,2 do ibuf[i]="" end
    self.ptr7 = timebase
    ibuf[1] = 0
    ...
    self.__complete = self:__EIBOpenVBusmonitorTS_Complete();
    return 0
end

function EIBConnection.EIBOpenVBusmonitorTS(self, timebase)
  local ibuf
    if DEBUG then print("Entering EIBConnection.EIBOpenVBusmonitorTS()") end
    if self:EIBOpenVBusmonitorTS_async (timebase) == -1  then 
      return -1
    end
    return self:EIBComplete()
  end
